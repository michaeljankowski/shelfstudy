import {
  BellRing, BookOpenCheck, BrainCircuit, Calculator, CalendarDays, ChevronRight,
  CircleStop, ClipboardList, FileText, FileText as SummaryIcon, Highlighter,
  Image as ImageIcon, Layers3, ListTodo, ListTree, MessageCircle, Network, Plus, Timer,
} from 'lucide-react';
...
const NOTE_TOOLS = [
  { label: 'Save to Notes', icon: BellRing },
  { label: 'Highlight Important', icon: Highlighter },
  { label: 'Extract Definitions', icon: BookOpenCheck },
  { label: 'Extract Formulas', icon: Calculator },
  { label: 'Create Outline', icon: ListTree },
];
const PRODUCTIVITY = [
...
const AVAILABLE_AI_TOOLS = new Set(['Explain this', 'Summarize', 'Study Guide', 'Flash Cards', 'Quiz Me', 'Connect Concepts']);
...
export default function SidebarSections({
  ...
  const handleAiTool = (item: string) => {
    switch (item) {
      case 'Explain this': return onChat();
      case 'Summarize': return onSummarize();
      case 'Study Guide': return onStudyPlan();
      case 'Flash Cards': return onFlashcards();
      case 'Quiz Me': return onQuiz();
      case 'Connect Concepts': return onConnectConcepts();
      default: return onComingSoon(item);
    }
  };
    ...
    <div className="sidebar-sections">
      <div className="sidebar-section">
        <div className="sidebar-section-header">
          ...
          <button type="button" className="sidebar-section-add" onClick={() => onOpenSourceTree('documents')} aria-label="Add source">
            <Plus size={16} />
          </button>
        </div>
        {recent.length === 0 ? <p className="sidebar-section-empty">No sources yet.</p> : (
...
function ToolGroup({ title, items, onAction, availableItems }: {
  title: string;
  ...
  return (
    <div className="sidebar-section">
      <div className="sidebar-section-header"><h4>{title}</h4></div>
